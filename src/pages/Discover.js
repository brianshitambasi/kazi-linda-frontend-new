...
import { 
  ...
  FaBriefcase, FaMapMarkerAlt, FaUserCheck, FaLeaf
} from 'react-icons/fa';
    ...
    <div style={styles.page}>
      <nav style={styles.nav}>
        <div style={styles.navLeft}>
          ...
          <div style={styles.searchBox}><FaSearch style={styles.searchIcon} /><input style={styles.searchInput} placeholder="Search people..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div>
        </div>
      ...
      <div style={styles.body}>
        <aside style={styles.leftSidebar}>
          ...
          <div style={styles.sidebarDivider} />
          <div style={styles.sidebarFooter}><FaLeaf /> Connect Â· Network Â· Grow<br />Â© {new Date().getFullYear()} KaziLinda</div>
        </aside>
        ...
        <aside style={styles.rightSidebar}>
          ...
          <div style={styles.sidebarFooter}><FaLeaf /> Connect with confidence<br />Â© {new Date().getFullYear()} KaziLinda</div>
        </aside>
      </div>
    </div>
  );
...
const styles = {
  ...
  sidebarNavItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '8px 8px', borderRadius: 8, border: 'none', background: 'transparent', width: '100%', fontSize: 14, cursor: 'pointer', color: colors.text },
  ...
  filterBar: { background: '#fff', borderRadius: 12, padding: '16px', marginBottom: 16, border: `1px solid ${colors.border}` },
  filterGroup: { position: 'relative', marginBottom: 12 },
  ...
  filterTab: { padding: '6px 12px', borderRadius: 20, border: `1px solid ${colors.border}`, background: colors.light, fontSize: 13, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' },
  filterTabActive: { background: colors.gradient, color: '#fff', borderColor: colors.primary },
  resultsCount: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 13, color: '#65676b', marginBottom: 12 },
  peopleGrid: { display: 'flex', flexDirection: 'column', gap: 12 },
  personCard: { background: '#fff', borderRadius: 12, padding: '16px', display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap', border: `1px solid ${colors.border}` },
  ...
  personName: { fontSize: 16, fontWeight: 600, color: colors.text, textDecoration: 'none', display: 'block', marginBottom: 4 },
  roleBadge: { background: colors.light, color: colors.text, fontSize: 11, padding: '2px 8px', marginRight: 6, borderRadius: 20 },
  ...
  messageBtn: { background: colors.light, border: `1px solid ${colors.border}`, color: colors.text },
  ...
  networkStat: { display: 'flex', justifyContent: 'space-around', textAlign: 'center' },
  networkNumber: { fontSize: 24, fontWeight: 700, color: colors.primary },
  countryList: { display: 'flex', flexDirection: 'column', gap: 8 },
  countryItem: { display: 'flex', justifyContent: 'space-between', padding: '8px', borderRadius: 8, border: 'none', background: 'transparent', fontSize: 14, cursor: 'pointer' },
  countryCount: { background: colors.light, color: colors.text },
  tipsCard: { background: colors.light, borderRadius: 12, padding: '16px', border: `1px solid ${colors.accent}` },
};
