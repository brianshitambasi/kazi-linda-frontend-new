...
import { 
  ...
  FaUsers, FaUserCheck, FaClock, FaEnvelope
} from 'react-icons/fa';
      ...
      <div style={styles.body}>
        <aside style={styles.leftSidebar}>
          ...
          <div style={styles.statsSummary}>
            ...
            <div><span style={styles.statValue}>{pendingCount}</span><span>Pending</span></div>
            <div><span style={styles.statValue}>{acceptedCount}</span><span>Accepted</span></div>
            <div><span style={styles.statValue}>{rejectedCount}</span><span>Rejected</span></div>
          </div>
          <div style={styles.sidebarDivider} />
          <div style={styles.sidebarFooter}>Review applications carefully<br />© {new Date().getFullYear()} KaziLinda</div>
        </aside>
        ...
        <main style={styles.feedCol}>
          ...
          <div style={styles.statsRow}>
            ...
            <div style={styles.statCardSmall}><div style={{ ...styles.statNumberSmall, color: '#f7b928' }}>{pendingCount}</div><div>Pending</div></div>
            <div style={styles.statCardSmall}><div style={{ ...styles.statNumberSmall, color: '#45bd62' }}>{acceptedCount}</div><div>Accepted</div></div>
            <div style={styles.statCardSmall}><div style={{ ...styles.statNumberSmall, color: '#e41e3f' }}>{rejectedCount}</div><div>Rejected</div></div>
          </div>
          ...
          <div style={styles.tableCard}>
            {applications.length === 0 ? (
              ...
              <table style={styles.table}>
                <thead><tr><th>Worker</th><th>Job Title</th><th>Skills</th><th>Applied On</th><th>Status</th><th>Actions</th></tr></thead>
                <tbody>
                  {applications.map(app => (
                    <tr key={app._id}>
                      ...
                      <td><div style={styles.skillsContainer}>{app.workerId?.skills?.slice(0, 3).map((s, i) => (<Badge key={i} bg="info" className="me-1">{s}</Badge>))}</div></td>
                      <td>{new Date(app.appliedAt).toLocaleDateString()}</td>
                      <td>{getStatusBadge(app.status)}</td>
                      <td><Button size="sm" variant="outline-info" onClick={() => { setSelectedApp(app); setShowModal(true); }}><FaEye /> Review</Button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          {selectedApp && (
            <>
              <div style={styles.modalUserInfo}>
                ...
                <div><h4>{selectedApp.workerId?.name}</h4><p><FaEnvelope /> {selectedApp.workerId?.email}</p></div>
              </div>
              <div style={styles.modalSection}><strong>Skills:</strong> <div>{selectedApp.workerId?.skills?.map((s, i) => (<Badge key={i} bg="info" className="me-1">{s}</Badge>))}</div></div>
              <div style={styles.modalSection}><strong>Experience:</strong> <p>{selectedApp.workerId?.experience || 'Not provided'}</p></div>
              <div style={styles.modalSection}><strong>Job Applied For:</strong> <p>{selectedApp.jobId?.title} - {selectedApp.jobId?.country}</p></div>
              <hr />
              <Form.Group><Form.Label>Feedback (optional)</Form.Label><Form.Control as="textarea" rows={3} value={feedback} onChange={(e) => setFeedback(e.target.value)} placeholder="Add feedback for the applicant..." /></Form.Group>
...
const styles = {
  ...
  tableCard: { background: '#fff', borderRadius: 12, overflow: 'auto', padding: '16px' },
  ...
  helpCard: { background: KL_BRAND_LIGHT, borderRadius: 12, padding: '16px', textAlign: 'center' }, helpBtn: { background: KL_BRAND, border: 'none', borderRadius: 6, padding: '8px 16px', width: '100%', marginTop: 12 },
  modalHeader: { borderBottom: '1px solid #dddfe2' }, modalTitle: { fontSize: 18, fontWeight: 600 }, modalBody: { padding: '20px' },
