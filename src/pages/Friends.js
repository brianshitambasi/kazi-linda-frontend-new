...
import { 
  ...
  FaBell, FaFacebookMessenger, FaEllipsisH,
  FaUsers, FaBookmark, FaClock,
      ...
      <nav style={styles.nav}>
        <div style={styles.navLeft}>
          ...
          <div style={styles.searchBox}>
            ...
            <input style={styles.searchInput} placeholder="Search friends..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
        </div>
      ...
      <div style={styles.body}>
        <aside style={styles.leftSidebar}>
          ...
          {leftLinks.map(({ icon: Icon, label, count, color, active }) => (
            <button key={label} style={{ ...styles.sidebarNavItem, ...(active ? styles.sidebarNavItemActive : {}) }}>
              ...
              {count !== undefined && <span style={styles.sidebarCount}>{count}</span>}
            </button>
          ))}
          <div style={styles.sidebarDivider} />
          <div style={styles.sidebarSectionTitle}>Friend Activity</div>
          <div style={styles.activityItem}><FaClock size={14} /><span>3 friends joined this week</span></div>
          <div style={styles.activityItem}><FaGlobe size={14} /><span>12 people you may know</span></div>
          <div style={styles.sidebarDivider} />
          <div style={styles.sidebarFooter}>Privacy · Terms · Safety Tips<br />© {new Date().getFullYear()} KaziLinda</div>
        </aside>

        <main style={styles.feedCol}>
          <div style={styles.headerCard}>
            <div style={styles.headerIcon}><FaUserFriends size={32} color={KL_BRAND} /></div>
            <div><h1 style={styles.headerTitle}>Friends</h1><p style={styles.headerDesc}>You have <strong>{friends.length}</strong> {friends.length === 1 ? 'connection' : 'connections'} on KaziLinda</p></div>
          </div>
          <div style={styles.tabsContainer}>
            <button style={{ ...styles.tab, ...(activeTab === 'friends' ? styles.tabActive : {}) }} onClick={() => setActiveTab('friends')}>All Friends ({friends.length})</button>
            <button style={{ ...styles.tab, ...(activeTab === 'suggestions' ? styles.tabActive : {}) }} onClick={() => setActiveTab('suggestions')}>Suggestions ({suggestions.length})</button>
          </div>
          {activeTab === 'friends' && (filteredFriends.length === 0 ? (
            <div style={styles.emptyState}><FaUserFriends size={64} color={KL_BRAND} /><h4>No connections yet</h4><p>Start following people to see them here!</p><Button onClick={() => setActiveTab('suggestions')} style={styles.primaryBtn}>Find Suggestions</Button></div>
          ) : (
            <div style={styles.friendsGrid}>{filteredFriends.map(friend => (
              <div key={friend._id} style={styles.friendCard}>
                <Link to={`/profile/${friend._id}`} style={styles.friendCardLink}>
                  ...
                  <div style={styles.friendMutual}><FaUserFriends size={12} /> 3 mutual friends</div>
                </Link>
                <div style={styles.friendActions}>
                  <Button as={Link} to={`/messages?user=${friend._id}`} variant="outline-primary" size="sm" style={styles.messageBtn}><FaEnvelope /> Message</Button>
                  <Button variant="outline-danger" size="sm" onClick={() => handleUnfollow(friend._id)} style={styles.unfollowBtn}>Unfollow</Button>
                </div>
          ...
          {activeTab === 'suggestions' && (suggestions.length === 0 ? (
            <div style={styles.emptyState}><FaUserPlus size={64} color={KL_BRAND} /><h4>No suggestions</h4><p>Check back later for more people to follow!</p></div>
          ) : (
            <div style={styles.suggestionsGrid}>{suggestions.map(suggestion => (
              <div key={suggestion._id} style={styles.suggestionCard}>
                ...
                <Button style={styles.followBtn} onClick={() => handleFollow(suggestion._id)}><FaUserPlus /> Follow</Button>
              </div>
            ))}</div>
          ))}
        </main>
...
const styles = {
  ...
  tabActive: { color: KL_BRAND }, friendsGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 },
  friendCard: { background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,.2)', position: 'relative', padding: '16px', textAlign: 'center' },
  friendCardLink: { textDecoration: 'none', display: 'block' }, friendName: { fontSize: 16, fontWeight: 600, marginTop: 12, marginBottom: 4, color: '#050505' },
  friendRole: { fontSize: 13, color: '#65676b', marginBottom: 4 }, friendLocation: { fontSize: 12, color: '#65676b', marginBottom: 4 },
  friendMutual: { fontSize: 12, color: KL_BRAND, marginTop: 8 }, friendActions: { display: 'flex', gap: 8, marginTop: 12 },
  messageBtn: { flex: 1, fontSize: 12, padding: '6px 8px' }, unfollowBtn: { flex: 1, fontSize: 12, padding: '6px 8px' }, friendBadge: { position: 'absolute', top: 12, right: 12, background: '#45bd62' },
  suggestionsGrid: { display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }, suggestionCard: { background: '#fff', borderRadius: 12, overflow: 'hidden', boxShadow: '0 1px 2px rgba(0,0,0,.2)', padding: '16px', textAlign: 'center' },
  followBtn: { background: KL_BRAND, border: 'none', borderRadius: 6, padding: '8px 16px', fontSize: 14, fontWeight: 600, width: '100%', marginTop: 12, color: '#fff' },
  ...
  rightCardHeader: { display: 'flex', alignItems: 'center', gap: 8, fontSize: 16, fontWeight: 600, marginBottom: 12, paddingBottom: 8, borderBottom: '1px solid #dddfe2' },
  emptyRequests: { textAlign: 'center', padding: '16px', color: '#65676b' }, onlineFriend: { display: 'flex', alignItems: 'center', gap: 12, padding: '8px 0' },
  onlineAvatar: { position: 'relative' }, onlineDot: { position: 'absolute', bottom: 0, right: 0, width: 10, height: 10, background: '#31a24c', borderRadius: '50%', border: '2px solid #fff' },
  onlineName: { fontSize: 14, fontWeight: 500, color: '#050505', textDecoration: 'none' }, onlineStatus: { fontSize: 12, color: '#65676b' },
  inviteCard: { background: `linear-gradient(135deg, ${KL_BRAND}22 0%, #fff 100%)`, borderRadius: 12, padding: '20px', textAlign: 'center', marginBottom: 16 },
  inviteBtn: { background: KL_BRAND, border: 'none', borderRadius: 6, padding: '8px 20px', fontWeight: 600, fontSize: 14, width: '100%', marginTop: 12 },
  emptyState: { textAlign: 'center', padding: '60px 20px', background: '#fff', borderRadius: 12, color: '#65676b' }, emptyStateSmall: { textAlign: 'center', padding: '20px', color: '#65676b' },
  primaryBtn: { background: KL_BRAND, border: 'none', padding: '8px 24px', borderRadius: 6, fontWeight: 600 },
};
