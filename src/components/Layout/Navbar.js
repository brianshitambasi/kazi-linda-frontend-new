...
import {
  ...
  FaSearch, FaEllipsisH, FaUsers, FaShieldAlt, FaChevronDown
} from 'react-icons/fa';
...
const Navbar = () => {
  ...
  const getDashboardPath = () => {
    if (!user) return '/';
    const map = { admin: '/admin/dashboard', employer: '/employer/dashboard', embassy: '/embassy/dashboard' };
  ...
  const allTabs = user ? [...NAV_TABS, { path: '/news', icon: FaNewspaper, label: 'Feed' }] : NAV_TABS;
    ...
    <>
      <nav style={s.nav}>
        <div style={s.navLeft}>
          ...
          <div style={s.searchBox}>
            ...
            <input style={s.searchInput} placeholder="Search KaziLinda…" value={searchVal} onChange={e => setSearchVal(e.target.value)} />
          </div>
        </div>

        <div style={s.navCenter}>
          {allTabs.map(tab => (
            <Link key={tab.path} to={tab.path} style={{ ...s.navTab, ...(isActive(tab.path) ? s.navTabActive : {}) }} title={tab.label}>
              <tab.icon size={22} style={{ color: isActive(tab.path) ? KL_BRAND : '#65676b' }} />
      ...
      {mobileOpen && (
        <div style={s.mobileDrawer}>
          {allTabs.map(tab => (
            <Link key={tab.path} to={tab.path} style={{ ...s.mobileLink, ...(isActive(tab.path) ? s.mobileLinkActive : {}) }} onClick={() => setMobileOpen(false)}>
...
const s = {
  ...
  logoBox: { width: 40, height: 40, borderRadius: '50%', background: '#f39c12', display: 'flex', alignItems: 'center', justifyContent: 'center', textDecoration: 'none', flexShrink: 0 },
  ...
  navTab: { position: 'relative', width: 80, height: 48, display: 'flex', alignItems: 'center', justifyContent: 'center', borderRadius: 10, textDecoration: 'none', transition: 'background .15s' },
  navTabActive: { background: '#fef9e7' },
  navTabLine: { position: 'absolute', bottom: 0, left: 0, right: 0, height: 3, background: '#f39c12', borderRadius: '2px 2px 0 0' },
  ...
  iconBtnLink: { position: 'relative', textDecoration: 'none', display: 'flex' },
  ...
  profileBtn: { display: 'flex', alignItems: 'center', gap: 6, background: '#f0f2f5', border: 'none', borderRadius: 24, padding: '4px 10px 4px 4px', cursor: 'pointer' },
  profileName: { fontSize: 15, fontWeight: 600, color: '#050505' },
  avatarInitials: { width: 36, height: 36, borderRadius: '50%', background: '#f39c12', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 700, fontSize: 14, flexShrink: 0 },
  loginBtn: { display: 'inline-flex', alignItems: 'center', background: '#fef9e7', color: '#f39c12', border: '1px solid #f39c12', borderRadius: 6, padding: '6px 14px', fontWeight: 700, fontSize: 14, textDecoration: 'none' },
  registerBtn: { display: 'inline-flex', alignItems: 'center', background: '#f39c12', color: '#fff', border: 'none', borderRadius: 6, padding: '6px 14px', fontWeight: 700, fontSize: 14, textDecoration: 'none' },
  dropdown: { position: 'absolute', top: 'calc(100% + 8px)', right: 0, background: '#fff', borderRadius: 12, boxShadow: '0 4px 20px rgba(0,0,0,.15)', minWidth: 180, zIndex: 400, overflow: 'hidden', padding: '8px 0' },
  dropdownHeader: { fontSize: 17, fontWeight: 700, color: '#050505', padding: '8px 16px 4px' },
  dropdownEmpty: { fontSize: 14, color: '#65676b', padding: '8px 16px 12px', textAlign: 'center' },
  dropdownProfileCard: { display: 'flex', alignItems: 'center', padding: '12px 16px' },
  dropdownDivider: { borderTop: '1px solid #f0f2f5', margin: '4px 0' },
  dropdownItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '10px 16px', fontSize: 15, color: '#050505', textDecoration: 'none', cursor: 'pointer' },
  dropdownIcon: { width: 32, height: 32, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  mobileDrawer: { position: 'fixed', top: 56, left: 0, right: 0, bottom: 0, background: '#fff', zIndex: 250, overflowY: 'auto', display: 'flex', flexDirection: 'column', padding: '8px 0', boxShadow: '0 4px 12px rgba(0,0,0,.15)' },
  mobileLink: { display: 'flex', alignItems: 'center', padding: '12px 20px', fontSize: 16, fontWeight: 500, color: '#050505', textDecoration: 'none', borderRadius: 0 },
  mobileLinkActive: { background: '#fef9e7', color: '#f39c12', fontWeight: 700 },
};
